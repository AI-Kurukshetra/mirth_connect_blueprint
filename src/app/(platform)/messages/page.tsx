...
export default function MessagesPage() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    let query = supabase
      .from("messages")
      .select("id, channel_id, connector_name, status, message_type, data_type, direction, processing_time_ms, created_at, raw_content, error_content, channels(name)", { count: "exact" });
    ...
    setLoading(false);
  }, [channelFilter, statusFilters, dateFrom, dateTo, contentSearch, page]);
  ...
  const channelName = (msg: Message) => {
    if (msg.channels && typeof msg.channels === "object" && "name" in msg.channels) {
      return (msg.channels as { name: string }).name;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between hb-animate-in">
        <div>
          <h1 className="text-2xl font-bold text-[var(--hb-text-primary)]">Message Browser</h1>
          <p className="text-[var(--hb-text-secondary)] mt-1">Search, filter, and inspect integration messages</p>
        </div>
      ...
      <div className="rounded-xl border border-[var(--hb-border)] bg-[var(--hb-surface)] p-4 hb-animate-in">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {/* Channel dropdown */}
          <div>
            <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-[var(--hb-text-ghost)] font-[family-name:var(--font-jetbrains)] block mb-1.5">Channel</label>
            <select
              value={channelFilter}
      ...
      <div className="rounded-xl border border-[var(--hb-border)] bg-[var(--hb-surface)] hb-animate-in">
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-[var(--hb-text-tertiary)] text-sm">Try adjusting your filters or wait for new messages to arrive.</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--hb-border)]">
                    ...
                    <th className="text-[9px] font-bold uppercase tracking-[0.2em] text-[var(--hb-text-ghost)] font-[family-name:var(--font-jetbrains)] text-left px-4 py-3">Channel</th>
                  ...
                  {messages.map((msg) => (
                    <>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm text-[var(--hb-text-secondary)]">{channelName(msg)}</td>
                            ...
                            <div className="p-5 space-y-4">
                              ...
                              <div className="grid grid-cols-4 gap-4">
                                ...
                                <div>
                                  ...
                                  <span className="text-sm text-[var(--hb-text-secondary)]">{channelName(msg)}</span>
                                </div>
                              </div>
                              {msg.raw_content && (
                                <div>
